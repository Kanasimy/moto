...
//echo "<pre>"; print_r($arElement); echo "</pre>";die();
		...
		<div class="bxr-element-container">
			...
			<div class="bxr-element-name" id="bxr-element-name-<?=$arElement["ID"]?>">
				<a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><?=$arElement["NAME"]?></a>
                    ...
                    <? 
                    $props = array();
                            foreach ($arElementParams[ 'DETAIL_PROPERTY_CODE'] as $propertyCode){
                               if(array_key_exists($propertyCode, $arElement['PROPERTIES']) && $arElement['PROPERTIES'][$propertyCode]['VALUE']){
                                   $props[$arElement['PROPERTIES'][$propertyCode]['NAME']] = $arElement['PROPERTIES'][$propertyCode]['VALUE'];
                               }
                            }
                     $props = array_slice($props, 0, 5);
                    if(count($props)):
                    ?>
                    <div class="bxr-element-props grid">
                    <table class="bxr-props-table">
                        <tbody>
                            <? foreach ($props as $name => $val):?>
                            <tr>
                                <td class="bxr-props-name"><span><?= $name?></span></td>
                                <td class="bxr-props-data"><span><?= $val?></span></td>
                            </tr>
                            <? endforeach;?>
                        </tbody>
                    </table>
...
$elementDraw->setAdditionalFile("JS", "/bitrix/templates/market_column/bxready/library/elements/ecommerce_v1/include/script.js", true);
$elementDraw->setAdditionalFile("CSS", "/bitrix/templates/market_column/bxready/library/elements/ecommerce_v1/include/style.css", false);
?>
