...
<?
		$arSort = array();
        $arSelect = array('ID', 'UF_SHOW_ELEMENTS');
        $arFilter = array(
            "IBLOCK_ID" => 24,
            "DEPTH_LEVEL" => 2,
            "SECTION_ID" => $arCurSection['ID'],
        ...
        $arSectionsShow = $arSectionsNotShow = array();
        while($ar_result = $res->GetNext())
        {
            if($ar_result['UF_SHOW_ELEMENTS'] == '1')
                $arSectionsNotShow[] = $ar_result['ID'];
            else
                $arSectionsShow[] = $ar_result['ID'];
        }
        
        if(count($arSectionsNotShow) > 0 && count($arSectionsShow) > 0)
        {
            global $arrFilter;
            $arrFilter['SECTION_ID'] = $arSectionsShow;
